  ...
  return (
    <section className="relative bg-linear-to-b from-black via-gray-900 to-black py-20 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="mb-16">
          <h3 className="text-2xl md:text-3xl font-bold text-gray-100 text-center mb-8">Infrastructure Components</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-4">
            {cablingFlow.slice(0, 5).map((item, index) => (
              <div
                key={index}
                className={`
                  bg-gray-900/50 border-2 rounded-xl p-4 md:p-6 
                  transition-all duration-300 backdrop-blur-sm
                  ${item.active ? 'border-blue-500/60 bg-blue-500/10' : 
                    item.highlight ? 'border-blue-400/60 bg-blue-400/10' : 
              ...
              >
                <h4 className={`
                  ...
                  ${item.active || item.highlight ? 'text-blue-300' : 'text-gray-300'}
                `}>
                  {item.label}
                </h4>
              </div>
          ...
          <div className="flex justify-center gap-4">
            {cablingFlow.slice(5).map((item, index) => (
              <div
                key={index + 5}
                className="bg-gray-900/50 border-2 border-gray-700/50 rounded-xl p-4 md:p-6 hover:border-blue-500/40 transition-all duration-300 backdrop-blur-sm hover:bg-gray-800/50 w-full md:w-64"
              >
                <h4 className="text-sm md:text-base font-bold text-center text-gray-300">
                  {item.label}
                </h4>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {/* Copper Cabling */}
          <div className="bg-gray-900/40 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-8 hover:border-blue-500/30 transition-all duration-300">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 rounded-xl bg-blue-500/10 flex items-center justify-center">
                <Cable className="w-6 h-6 text-blue-400" />
              </div>
              <h3 className="text-2xl font-bold text-gray-100">Copper Cabling</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {copperCabling.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-4 hover:border-blue-500/40 hover:bg-gray-800/60 transition-all duration-300 group"
                  >
          ...
          <div className="bg-gray-900/40 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-8 hover:border-blue-500/30 transition-all duration-300">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-2xl font-bold text-gray-100">Fiber Cabling</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
